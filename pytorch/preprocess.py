...
def preprocess_dataset(dataset_path, output_path='preprocessed.h5'):
    ...
    for i, (video_data) in enumerate(loader):
        ...
        for frame_data in frame_group:
            ...
            frame_times.append(frame_time.numpy()[0])
            frame_indices.append(index.numpy()[0])

        face_encode = np.array(face).astype(np.uint8)
        eye_l_encode = np.array(eye_l).astype(np.uint8)
        eye_r_encode = np.array(eye_r).astype(np.uint8)
        metadata_encode = np.array(metadata_record).astype(np.float32)
        frame_time_encode = np.array(frame_times).astype(np.float32)
        ...
        with h5py.File(output_path, 'a') as output:
            ...
            output.create_dataset(f'{subject}/{trial}_{pose}/frame_times', data=frame_time_encode)
